...
class AttackHitbox:
    ...
    def update_hitbox(self, attack_type, current_frame_index) -> None:
        """ Atualiza a hitbox de ataque com base no frame atual e na direção do jogador. """
        attack_width, attack_height = self._get_attack_hitbox_size(attack_type, current_frame_index)
    ...
    def _get_attack_hitbox_size(self, attack_type, current_frame_index) -> tuple:
        """ Retorna o tamanho da hitbox de acordo com o tipo de ataque atual. """
        return self._get_sword_hitbox_size(current_frame_index) if attack_type == 1 else self._get_cannon_hitbox_size(current_frame_index)
            

    def _get_sword_hitbox_size(self, current_frame_index) -> tuple[int, int]:
        """ Retorna o tamanho da hitbox baseado no frame atual. """
        if 3 <= current_frame_index < 6:
            return (310, 250)
        else:
            return (100, 150)
        

    def _get_cannon_hitbox_size(self, current_frame_index) -> tuple[int, int]:
        """ Retorna o tamanho da hitbox baseado no frame atual. """
